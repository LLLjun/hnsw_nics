...
def space_explore(mode):
    ...
    for dataname in datasets:
        ...
        for k in k_list:
            for efs in efs_list:
                if mode == "train":
                    os.system("cd build && make main && cp main main_c3_base_10m_100")

                    for efc in max_efc:
                        for m in max_m:
                            command = "cd build && ./main_c3_base_10m_100 " + stage + " " + dataname + " " + format + " " + str(datasize) + " " + str(efc) + " " + str(m) + " " + str(k) + " " + str(efs)
                            os.system(command)
